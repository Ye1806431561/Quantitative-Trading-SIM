...
## 工程骨架与基础实现文件作用（第 3-11 步）
- `src/core/*.py`：核心业务域实现入口（账户、订单、撮合、数据库、领域模型校验）；其中 `database.py` 已落地生命周期管理与 schema 初始化（六表、约束、索引），`enums.py`/`validation.py`/`account.py`/`order.py`/`trade.py`/`position.py`/`candle.py`/`strategy_run.py` 已完成领域模型与校验规则（`validation.py` 已修复 `require_timestamp()` 兼容 SQLite `datetime` 对象），`account_service.py` 已实现账户初始化、余额管理、持仓恢复与总资产估值，第 12 步以后可直接复用；`matching.py` 等其余模块为后续 Phase 1-2 承接点。
