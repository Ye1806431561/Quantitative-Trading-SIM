...
class BacktestEngine:
    ...
    def run(self, request: BacktestRunRequest) -> BacktestRunResult:
        ...
        # Wrap strategy to collect individual trade records via notify_trade() (step 28)
                ...
                def notify_trade(self, trade: bt.Trade) -> None:
                    ...
                    trade_records.append(
                        TradeRecord(
                            ...
                            exit_price=trade.pnlcomm / abs(trade.size) + trade.price
                            if abs(trade.size) > 0
                            else trade.price,
                            pnl_gross=trade.pnl,
        ...
        # Mount standard analyzers (step 27)
    ...
    @staticmethod
    def _validate_strategy_class(strategy_class: type[bt.Strategy]) -> type[bt.Strategy]:
        if not isinstance(strategy_class, type) or not issubclass(strategy_class, bt.Strategy):
            raise BacktestEngineError("strategy_class must be a subclass of backtrader.Strategy")
